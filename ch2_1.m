...
noise_power=(signal_power*fs)/(snr_lin*4*2*f0);%求出噪声方差（噪声均值为0）
...
xlabel('f');
ylabel('信道中信号频谱');
[H,f]=bp_f(length(sam),fc-2*f0,fc+2*f0,df1,fs,1);%求带通滤波器
subplot(326); 
...
%画出理想低通滤波器
figure(4)
[LPF,f]=lp_f(length(der),fc+2*f0,df1,fs,2);%求低通滤波器
subplot(322)
